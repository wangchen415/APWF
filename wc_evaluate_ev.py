...
def Beta(Qm,Qs):
    ...
    Qs=Qs.astype(np.float64).dropna()
    try:
        beta =  (sum(Qs)/len(Qs)) / (sum(Qm)/len(Qm))
        # print(kge)
    except:
        beta=0
    return beta
...
def Gamma(Qm,Qs):
    ...
    try:
        # r = ((Qm - (sum(Qm)/len(Qm))) * (Qs - (sum(Qs)/len(Qs)))).sum() / (math.sqrt(((Qm - (sum(Qm)/len(Qm))) ** 2).sum() * ((Qs - (sum(Qs)/len(Qs))) ** 2).sum()))
        gamma = (np.nanstd(Qs) / np.nanstd(Qm))
        # print(kge)
    except:
        gamma=0
    return gamma
...
def process_file(args):
    ...
    if modle.split('-')[0]=='flow':
        ...
        for k,df  in simflow.items():
            ...
            cc['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=CC(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            beta['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=Beta(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            gamma['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=Gamma(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            ...
            for key, value in cc.items():
                first_char = key.split('-')[0]
                if first_char in cc_flow:
                    cc_flow[first_char] += value*weig
                else:
                    cc_flow[first_char] = value*weig
            #
            beta_flow = {}
            for key, value in beta.items():
                first_char = key.split('-')[0]
                if first_char in beta_flow:
                    beta_flow[first_char] += value*weig
                else:
                    beta_flow[first_char] = value*weig
            #
            gamma_flow = {}
            for key, value in gamma.items():
                first_char = key.split('-')[0]
                if first_char in gamma_flow:
                    gamma_flow[first_char] += value*weig
                else:
                    gamma_flow[first_char] = value*weig
        r_square,nes,pbias,kge,cc,beta,gamma=r_square_flow,nes_flow,pbias_flow,kge_flow,cc_flow,beta_flow,gamma_flow
    ...
    #
        ...
        for k,df  in simET.items():
            ...
            stderr_, rho, snr_db_, sensitivity_ = tc.etc(q_hat)
            print(stderr_)
            # print('%d-%s'%(int(os.path.split(filename)[-1][7:10]),k),stderr[0])
            r_square['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=rho[0]**2
            stderr['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=stderr_[0]
            snr_db['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=snr_db_[0]
            sensitivity['%d-%s'%(int(os.path.split(filename)[-1][7:10]),k)]=sensitivity_[0]
            ...
            for key, value in stderr.items():
                first_char = key.split('-')[0]
                if first_char in stderr_ET:
                    stderr_ET[first_char] += value*weig
                else:
                    stderr_ET[first_char] = value*weig
            snr_db_ET = {}
            for key, value in snr_db.items():
                first_char = key.split('-')[0]
                if first_char in snr_db_ET:
                    snr_db_ET[first_char] += value*weig
                else:
                    snr_db_ET[first_char] = value*weig
            sensitivity_ET = {}
            for key, value in sensitivity.items():
                first_char = key.split('-')[0]
                if first_char in sensitivity_ET:
                    sensitivity_ET[first_char] += value*weig
                else:
                    sensitivity_ET[first_char] = value*weig
        r_square,stderr,snr_db,sensitivity=r_square_ET,stderr_ET,snr_db_ET,sensitivity_ET
    if modle.split('-')[0]=='flow':
        return {"r_square": r_square, "nes": nes, "pbias": pbias, "kge": kge,"cc":cc,"beta":beta,"gamma":gamma}
    elif modle.split('-')[0]=='TC':
        return {"r_square": r_square, "stderr": stderr, "snr_db":snr_db,"sensitivity":sensitivity}
