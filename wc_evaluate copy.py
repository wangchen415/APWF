...
def process_file(args):
    ...
    if modle.split('-')[0]=='flow':
        ...
        for k,df  in simflow.items():
            ...
            aaa=np.sum(merged['FLOW_OUT_s'].values)
            end['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=aaa
            r_square['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=r2(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            nes['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=Nes(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            pbias['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=PBIAS(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            kge['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=KGE(merged['FLOW_OUT_m'],merged['FLOW_OUT_s'])
            ...
            for key, value in end.items():
                first_char = key.split('-')[0]
                if first_char in end_:
                    end_[first_char] += value*weig
                else:
                    end_[first_char] = value*weig
        r_square,nes,pbias,kge,end=r_square_flow,nes_flow,pbias_flow,kge_flow,end_
        ...
        for k,df  in simET.items():
            ...
            r_square['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=r2(merged['ET_m'],merged['ET_s'])
            nes['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=Nes(merged['ET_m'],merged['ET_s'])
            pbias['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=PBIAS(merged['ET_m'],merged['ET_s'])
            kge['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=KGE(merged['ET_m'],merged['ET_s'])
            # r_square_dict = pd.Series(r_square).rename('r_square')
            # nes_dict = pd.Series(nes).rename('nes')
            # pbias_dict = pd.Series(pbias).rename('pbias')
            # kge_dict = pd.Series(kge).rename('kge')
    ...
    #
        ...
        for k,df  in simET.items():
            ...
            # print(rho**2)
            r_square['%d-%s'%(int(os.path.split(filename)[-1][7:11]),k)]=rho[0]**2
        ...
        r_square,nes,pbias,kge=r_square_ET,nes_ET,pbias_ET,kge_ET
    print(kge,end)
    return {"r_square": r_square, "nes": nes, "pbias": pbias, "kge": kge,"end":end}
